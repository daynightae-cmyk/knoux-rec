...
contextBridge.exposeInMainWorld("knouxRec", {
  ...
  recovery: {
    ...
    recover: (sessionId) => invoke("recovery:recover", sessionId),
    discard: (sessionId) => invoke("recovery:discard", sessionId),
  },
  project: {
    ...
    exportSrt: (recordingId) => invoke("project:export-srt", recordingId),
  },
  recording: {
