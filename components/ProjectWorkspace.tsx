...
export default function ProjectWorkspace(props: ProjectWorkspaceProps) {
  ...
  const exportProject = async () => {
    ...
    setExportMessage(null);
    try {
      const result = await window.knouxRec.project.export({ recordingId: selectedRecordingId, format: exportFormat });
      setExportState("complete");
  ...
  const renderedProject = project && selectedRecord ? <>
    ...
    {panel === "export" && <ExportCard project={project} t={t} dirty={dirty} saving={saving} exportState={exportState} exportMessage={exportMessage} onExport={exportProject} exportFormat={exportFormat} onFormatChange={setExportFormat} />}
  </> : null;
...
function ExportCard({ project, t, dirty, saving, exportState, exportMessage, onExport, exportFormat, onFormatChange }: { project: KnouxProject; t: Copy; dirty: boolean; saving: boolean; exportState: string; exportMessage: string | null; onExport: () => void; exportFormat: "mp4" | "webm"; onFormatChange: (format: "mp4" | "webm") => void }) {
  const range = selectedRange(project);
  return <section className="card export-card"><p className="eyebrow">{t.exportFormat}</p><h2>{t.export}</h2><label className="export-format">{t.exportFormat}<select value={exportFormat} onChange={(event) => onFormatChange(event.target.value as "mp4" | "webm")}><option value="mp4">{t.formatMp4}</option><option value="webm">{t.formatWebm}</option></select></label><div className="export-facts"><div><span>{t.configuredRange}</span><strong>{formatMs(range.startMs)} — {formatMs(range.endMs)}</strong></div><div><span>{t.duration}</span><strong>{formatMs(range.endMs - range.startMs)}</strong></div></div><p className="muted">{t.exportNote}</p><p className="muted">{t.exportsTo}</p>{dirty && <p className="export-error">{t.unsaved}</p>}<button className="record-button" disabled={dirty || saving || exportState === "exporting"} onClick={() => void onExport()}>{exportState === "exporting" ? t.exporting : t.exportMp4}</button>{exportMessage && <p className={exportState === "failed" ? "export-error" : "export-result"}>{exportMessage}</p>}</section>;
}
