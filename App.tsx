...
const copy = {
  en: {
    ...
    last: "Latest recording", noRecording: "No recording has been finalized in this session.", open: "Open", reveal: "Show in folder", delete: "Delete", exportMp4: "Export MP4", exported: "MP4 export completed locally and was verified.", recordings: "Saved recordings", load: "Refresh library", emptyLibrary: "Your finalized local recordings will appear here.", searchLibrary: "Search recordings", sortNewest: "Newest", sortOldest: "Oldest", sortDuration: "Longest", sortSize: "Largest", sortName: "Name", timelineZoom: "Timeline zoom", resetZoom: "Reset zoom", created: "Created", duration: "Duration", sizeLabel: "Size", muxed: "System audio muxed", sidecar: "System WAV source retained",
  ...
  ar: {
    ...
    last: "أحدث تسجيل", noRecording: "لم يتم إنهاء أي تسجيل في هذه الجلسة.", open: "فتح", reveal: "إظهار في المجلد", delete: "حذف", exportMp4: "تصدير MP4", exported: "اكتمل تصدير MP4 محلياً وتم التحقق منه.", recordings: "التسجيلات المحفوظة", load: "تحديث المكتبة", emptyLibrary: "ستظهر هنا تسجيلاتك المحلية التي تم إنهاؤها.", searchLibrary: "بحث في التسجيلات", sortNewest: "الأحدث", sortOldest: "الأقدم", sortDuration: "الأطول", sortSize: "الأكبر", sortName: "الاسم", timelineZoom: "تكبير خط الزمن", resetZoom: "إعادة الضبط", created: "تاريخ الإنشاء", duration: "المدة", sizeLabel: "الحجم", muxed: "تم دمج صوت النظام", sidecar: "تم الاحتفاظ بمصدر WAV للنظام",
...
export default function App() {
  ...
  const [librarySort, setLibrarySort] = useState<"newest" | "oldest" | "duration" | "size" | "name">("newest");
  ...
  const libraryRecords = useMemo(() => {
    ...
    const filtered = records.filter((record) => `${record.fileName} ${record.media?.video?.codec || ""}`.toLocaleLowerCase().includes(query));
    return [...filtered].sort((left, right) => {
      if (librarySort === "oldest") return Date.parse(left.createdAt) - Date.parse(right.createdAt);
      if (librarySort === "duration") return right.durationMs - left.durationMs;
      if (librarySort === "size") return right.sizeBytes - left.sizeBytes;
      if (librarySort === "name") return left.fileName.localeCompare(right.fileName, locale);
      return Date.parse(right.createdAt) - Date.parse(left.createdAt);
    });
  }, [libraryQuery, librarySort, locale, records]);
